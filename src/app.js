document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderWeek() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // Verifica se há escritas no dia (highlight ou qualquer horário)
      const hasHighlight = drawingStore[`${ds}-highlight`]?.strokes?.length > 0;
      let hasWriting = hasHighlight;
      if (!hasWriting) {
        for (let h = 0; h < 24; h++) {
          if (drawingStore[`${ds}-${h}`]?.strokes?.length) { hasWriting = true; break; }
        }
      }
      ...
      html += `</div>`;
      // Meio: indicador visual de escrita
      html += `<div class="flex-1 flex flex-col items-center justify-center gap-2 p-2">`;
      if (hasWriting) {
        html += `<div class="flex flex-col items-center gap-1">`;
        html += `<span class="material-symbols-outlined text-[24px] text-primary/50" style="font-variation-settings:'FILL' 1">ink_pen</span>`;
        html += `<span class="text-[9px] text-primary/40 font-body-sm">escrito</span>`;
        html += `</div>`;
      } else {
      ...
      html += `</div>`;
      // Fim: linha sutil
      html += `<div class="h-1 shrink-0 ${isToday ? 'bg-primary' : hasWriting ? 'bg-primary/30' : 'bg-outline-variant/20'}"></div>`;
      html += `</div>`;
    }
